...
Throughout evolution, chlorophyll-based photosynthetic organisms have colonized different environments and adapted to different habitat conditions, ranging from aquatic algae to parasitic and carnivorous plants. This study aims to identify synteny and rearrangements among representative algae and photosynthetic, parasitic, and carnivorous plants, and to determine which gene clusters related to metabolic pathways remain conserved within syntenic blocks. It is suggested that a representative species from each group, with a small genome, be selected and that the analysis of gene clusters be limited to functional groups to encompass processes conserved over time.

## 1. Introduction

The evolutionary history of land plants originated in the sea, within the charophyte green algae (Ma, 2022). This evolutionary process involved not only the gain and co-option of genes, but also the loss of others. Importantly, there are studies on gene families among photosynthetic species, such as *Arabidopsis thaliana* and *Chlamydomonas reinhardtii*. However, these have not been compared with the land plants *Cuscuta australis* (a parasitic plant) and *Roridula gorgonias* (a carnivorous plant). As an indication, prior reports show that gene families conserved in terrestrial plants tend to be linked to stress responses and to the transport of ions and metabolites (Ma, 2022).

Here, we evaluated the shared syntenic blocks in these four species: three terrestrial plants (*Arabidopsis thaliana*, *Cuscuta australis* and *Roridula gorgonias*) and a unicellular alga (*Chlamydomonas reinhardtii*).

Our objective was to determine the synteny of conserved metabolic pathways among these photosynthetic species. It is expected to identify the conservation of some key metabolic pathways among the analyzed organisms, such as: ion transport (such as K, Fe, Cu, etc.), cell cycle regulation, abiotic stress response mechanisms (particularly relevant in terrestrial plants), photosynthesis (possibly partially in *Cuscuta australis*), metabolism of secondary compounds for biotic adaptation and embryonic development (Guo, 2012; Sun et al., 2018; Ma, 2022; Fleck et al., 2023).

To achieve these objectives, a series of bioinformatics tools were used. Genomic data were downloaded from public databases such as NCBI and Dryad. The quality of the genomes was assessed using the tools **QUAST** and **BUSCO**, available in Galaxy. Programming languages such as Python, Bash and R were used for file editing, data processing, and analysis. Annotation and identification of conserved orthologous genes were performed using **eggNOG-mapper** and **OrthoFinder**, respectively. R was used to graphically represent the results, complemented by Excel in specific cases. Syntenic blocks were determined using **MCScanX**, and their visualization was managed with **SynVisio**. Finally, the enrichment of GO terms in the syntenic blocks was analyzed using the **DAVID** and **PANTHER** platforms.
...
- Define genome-exclusion criteria due to computational limitations  
...
> **Note:** From this point on, several Python/R scripts are referenced with screenshots as examples. Full scripts are available by name in the `scripts` folder.


### Guide flowchart  
The numbers correspond to the section where the named topic is discussed. QUAST and BUSCO are not considered.

![][image2]

### 3.1. Sequence selection parameters

For the sequence selection process, the following criteria were considered: photosynthetic organism, availability of the genome, proteins and annotations in databases such as NCBI or Dryad, and genome size between 100 and 300 Mb (small), for efficient computational management given our limited resources.
...
| Downloaded documents | Genome sequences (FASTA) → fna; Annotations → GFF; Proteins (FASTA)→ faa | Genome sequences (FASTA) → fna; Annotation Functions → GFF; Proteins (FASTA)→ faa | Genome sequences (FASTA) → fna; Annotation Functions → GFF; Proteins (FASTA)→ faa | Genome sequences (FASTA) → fna; Sequence and annotations → GBFF; No protein file available |
...
Final set of genomes used (Table 1):
...
4. All other parameters were left as default.
...
### 3.3. Assigning functions to protein sequences
...
1. Working parameters  
   ...
   4. Search engine: DIAMOND  
   5. Diamond Sensitivity Mode: Sensitive  
   6. All other parameters were left as default.
...
- **Visualizing results:** We used the script **figures.Rmd** in R for the results shown in figures 4 and 5, as well as table 2.
...
# After adding the prefixes to the genes, reorder the columns  
...
- Afterwards, the script get_length_chromosomes_FASTA.py was used to calculate the lengths of each chromosome for each species and visualize the largest genomes with SynVisio (since most are small chromosomes). We also inspected where the largest syntenic blocks might occur.
...
1. We removed everything after the first token in each header, keeping only the accession.
...
MCScanX first uses the BLASTP results to detect homologous genes and then identifies nearby homologs (within a user-defined window) to build collinear syntenic blocks.
...
* -w (overlap window): Maximum distance (in number of genes) between matching BLAST genes to be collapsed as part of the same block. In the “second” relaxed run (step 2), this parameter was increased to allow generating syntenic blocks even between relatively distant BLAST homologous genes (up to 10 genes distant).  
* -m (maximum gaps): Maximum number of gaps (missing or non-homologous genes) allowed within a syntenic block. In the relaxed run in step 2, this parameter was increased to 50 to allow the syntenic block to be maintained even if there are up to 50 non-homologous genes.  
* -s (minimum match size): Minimum number of genes required to consider a block syntenic. The value of this parameter was reduced to 3 in the second run, to allow calling smaller blocks (up to a minimum of 3 genes).

It is important to highlight that although both forms were worked with, both with stricter parameters by default (step 1), and with relaxed ones (step 2), after the inspection of the data carried out in “MCScanX data processing”, we ultimately used the default (stricter) parameters, which yielded more consistent syntenic blocks with coherent (similar) GO functions.
...
* This all.collinearity file, along with the GFF accepted by MCScanX, served as input for [SynVisio](https://synvisio.github.io/#/).
...
10. Next, the first result (step 7) of the GO count in each interspecies syntenic block was used to easily locate some large syntenic blocks in SynVisio, visualize them, and be able to verify that they are enriched in certain GO terms, as seen in the image below. Initial enrichment inspection was visual (based on the most frequent GO descriptions per syntenic block). We then collected the locus_tags for proteins in blocks of interest and submitted them to [DAVID](https://davidbioinformatics.nih.gov/tools.jsp), which confirmed GO-term enrichment for the specific syntenic block (Figure 11).

11. Additionally, we compared all syntenic blocks between each species pair. As mentioned above, because only between *Arabidopsis* and *Cuscuta* a good number of conserved blocks were obtained, we decided to focus on these species. Then, all syntenic blocks were chosen between *Arabidopsis* and *Cuscuta*, we extracted all of their locus_tags (from step 9) and entered them into PANTHER via the [Gene Ontology portal](https://geneontology.org/). DAVID was not used because it did not accept such a large number of locus_tags. The result was a table, which was downloaded and processed with the R script **figures.Rmd**, to filter out those with the highest Fold_Enrichment and p-value adjusted for multiple testing. The output was the following HTML file.
...
**Figure 4.** Percentage of sequences from each species assigned to singletons (orthologous groups of a single sequence), orthologous groups of two sequences, and orthologous groups with more than three sequences.
![][image37]  
...
Share of Collinear Genes indicates that of the total genes analyzed, \~16% are found in collinear blocks, that is, conserved genomic regions. The colored fragmented lines represent chromosomes, while gray lines join common syntenic blocks.
...
**Figure 7.** Bar chart of GO-term counts (number of genes) per species.

![][image42]

**Figure 8.** GO terms enriched in syntenic blocks between *Arabidopsis* and *Cuscuta* I.  
...
**Figure 9.** GO terms enriched in syntenic blocks between *Arabidopsis* and *Cuscuta* II.

![][image44]

GO terms associated with ionic transport are observed

**Figure 10.** GO terms enriched in syntenic blocks between *Arabidopsis* and *Cuscuta* III. 
...
* *Cuscuta* and *Roridula* show fewer proteins than *Arabidopsis*, including reductions in photosynthesis-related proteins. 
...
* Cannon, S. B., Mitra, A., Baumgarten, A., Young, N. D., & May, G. (2004). The roles of segmental and tandem gene duplication in the evolution of large gene families in Arabidopsis thaliana. BMC Plant Biology, 4, 10. DOI: 10.1186/1471-2229-4-10

* Dryad. (2024). Who we are. https://datadryad.org/about

* Fleck, S. J., & Jobson, R. W. (2023). Molecular phylogenomics reveals the deep evolutionary history of carnivory across land plants. Plants, 12(19), 3356. DOI: 10.3390/plants12193356

* Guo, Y.-L. (2013). Gene family evolution in green plants with emphasis on the origination and evolution of Arabidopsis thaliana genes. The Plant Journal, 73(6), 941–951. DOI: 10.1111/tpj.12089

* Hartmann, S., Preick, M., Abelt, S., Scheffel, A., & Hofreiter, M. (2020). Annotated genome sequences of the carnivorous plant Roridula gorgonias and a non-carnivorous relative, Clethra arborea. BMC Research Notes, 13, 426. DOI: 10.1186/s13104-020-05254-4

* Ma, J., Wang, S., Zhu, X., Sun, G., Chang, G., Li, L., Hu, X., Zhang, S., Zhou, Y., Song, C.-P., & Huang, J. (2022). Major episodes of horizontal gene transfer drove the evolution of land plants. Molecular Plant, 15(5), 857–871. DOI: 10.1016/j.molp.2022.02.001

* Sun, G., Xu, Y., Liu, H., et al. (2018). Large-scale gene losses underlie the genome evolution of the parasitic plant Cuscuta australis. Nature Communications, 9, 2683. DOI: 10.1038/s41467-018-04721-8
